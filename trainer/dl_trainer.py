...
def lr_decay_noam(config):
    return lambda t: (
        10.0
        * config.rnn.hidden_dim**-0.5
        * min((t + 1) * config.learning_rate_warmup_steps**-1.5, (t + 1) ** -0.5)
    )
...
class Trainer:
    def __init__(self, config, logger, dataloader, model, device, k):
        ...
        if self.config.use_lr_decay:
            ...
            lr_scheduler = optim.lr_scheduler.LambdaLR(
                self.opt, lrd_func(self.config), last_epoch=-1
            )
        ...
        self.model_file = path.join(
            self.config.output_dir, self.config.model, self.file_name
        )
    ...
    def categorical_accuracy(self, preds, y):
        ...
        max_preds = preds.argmax(dim=1, keepdim=True).squeeze(
            1
        )  # Shape -> (batch_size)
    ...
    def train(self, model, iterator, optimizer, criterion):
        ...
        for coll in iterator:
            ...
            predictions = (
                model(X, mask) if self.config.model in ["mbert", "muril"] else model(X)
            )  # Shape -> (batch_size, 2)
            gold = Y

            gold = (
                gold.squeeze(1) if len(gold.shape) > 1 else gold
            )  # Shape -> (batch_size)
            ...
            y_pred = predictions.argmax(dim=1, keepdim=True).squeeze(
                1
            )  # Shape -> (batch_size)
        ...
        assert len(gold_list) == len(
            pred_list
        ), f"The gold list has length {len(gold_list)} while the predicted list has length {len(pred_list)}."
    ...
    def fit(self):
        ...
        for epoch in range(0, self.epochs):
            ...
            train_loss, train_acc, train_gold_list, train_pred_list = self.train(
                self.model, tqdm_t, self.opt, self.loss_fn
            )
            valid_loss, valid_acc, valid_gold_list, valid_pred_list = self.evaluate(
                self.model, tqdm_v, self.loss_fn
            )

            # Precision, Recall, F1 Score and ROC-AUC Score.
            train_pr, train_rec, train_f1, train_auc = compute_prec_rec_f1(
                train_gold_list, train_pred_list, self.config.average
            )
            valid_pr, valid_rec, valid_f1, valid_auc = compute_prec_rec_f1(
                valid_gold_list, valid_pred_list, self.config.average
            )
            ...
            if valid_loss < best_valid_loss:
                ...
                if self.verbose:
                    self.logger.info(
                        f"No improvement in validation loss. Tolerance count : {counter}\n"
                    )
            ...
            if counter >= self.early_max_patience:
                if self.verbose:
                    self.logger.info(
                        f"Training stopped because maximum tolerance of {self.early_max_patience} reached."
                    )
                break
        ...
        cache_filename = path.join(
            cache_dir,
            f"cache_{self.config.model}_{self.config.train_type}_Fold_{str(self.k)}.csv",
        )
    ...
    def predict(self):
        ...
        """
        evaluate = Evaluator(
            self.config, self.logger, self.model, self.dataloader, self.model_name
        )

        self.model.eval()
        tqdm_tst = tqdm(iter(self.test_dl), leave=False, total=self.test_dlen)
        test_loss, test_acc, gold_list, pred_list = self.evaluate(
            self.model, tqdm_tst, self.loss_fn
        )
        self.logger.info(
            f"Test. Loss: {test_loss:.3f} || Test Acc: {test_acc*100:.4f}%"
        )
