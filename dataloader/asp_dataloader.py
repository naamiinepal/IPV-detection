...
class AspectExtractionCorpus_Transformer:
    def __init__(self, input_directory, tokenizer, max_len) -> None:
        ...
        self.train_ds = AspectDataset(
            os.path.join(self.input_directory, "train.txt"), tokenizer, max_len
        )
        self.labels_to_ids = self.train_ds.labels_to_ids
        self.val_ds = AspectDataset(
            os.path.join(self.input_directory, "val.txt"),
            self.tokenizer,
            self.max_len,
            self.labels_to_ids,
            is_train=False,
