...
def create_batches_twitter(root_dir, filename, save_dir):
    ...
    df_overall = pd.read_excel(
        r"D:\ML_projects\IPV-Project\annotation\data\overall.xlsx"
    )
    ...
    for k, ii in tqdm(enumerate(range(0, len(text), 10), 1)):
        ...
        if k % 2 == 0:
            batch.to_csv(
                join(save_dir_shr, save_filename),
                header=None,
                index=None,
                encoding="utf-8",
            )
            ...
            batch.to_csv(
                join(save_dir_krn, save_filename),
                header=None,
                index=None,
                encoding="utf-8",
            )
...
if __name__ == "__main__":
    create_batches_twitter(
        root_dir=r"D:\ML_projects\IPV-Scraper\results\second_lot",
