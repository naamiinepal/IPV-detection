...
def kfold_split_aspect(args, logger):
    ...
    # Read raw data.
    data = pd.read_csv(
        join(args.input_dir, "asp_extraction.tsv"),
        header=None,
        delimiter="\t",
        encoding="utf-8",
    )
    ...
    if args.verbose:
        ...
        logger.info(
            f"Number of duplicates removed : {shape_before[0] - shape_after[0]}.\n"
        )
    ...
    data["labels"] = data["labels"].apply(
        lambda label: np.nan if all(element == "O" for element in label) else label
    )
    ...
    if args.verbose:
        ...
        logger.info(f"Shape after: {shape_after}.")
        logger.info(
            f'Number of sentences with only "O" tags removed : {shape_before[0] - shape_after[0]}.\n'
        )
...
def kfold_split_polarity(args, logger):
    ...
    # Load IPV examples.
    ipv = pd.read_csv(
        join(args.input_dir, "IPV_sents.tsv"), delimiter="\t", encoding="utf-8"
    )
    ...
    # Load IPV examples.
    non_ipv = pd.read_csv(
        join(args.input_dir, "non-IPV_sents.tsv"), delimiter="\t", encoding="utf-8"
    )
