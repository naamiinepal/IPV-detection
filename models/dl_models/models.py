...
class RNN(nn.Module):
    def __init__(self, config, dataloader):
        ...
        if config.pretrained:
            self.embedding = nn.Embedding.from_pretrained(
                dataloader.weights, freeze=self.config.freeze_embedding
            )
        else:
    ...
    def forward(self, text):  # TAKE IN AC AND AT LATER.
        text = text.permute(
            1, 0
        )  # BEFORE: (batch_size, sent_len) --> AFTER: (sent_len, batch_size)
        # at = at.permute(1, 0)
        # ac = ac.permute(1, 0)
        ...
        )  # Shape -> (batch size, hid dim * num directions)
...
class CNN(nn.Module):
    ...
    def forward(self, text):
        ...
        assert (
            text.shape[-1] >= limit
        ), f"The length of the given sequence is {text.shape[-1]} which is less than the kernel size {limit}."
        ...
        conved = [
            F.relu(conv(self.dropout(embedded))).squeeze(3) for conv in self.convs
        ]
...
class BertClassifier_Linear(nn.Module):
    def __init__(self, BERT_MODEL_NAME, output_dim=2, dropout=0.5):
        ...
        self.linear = nn.Sequential(
            nn.Linear(768, 512), nn.ReLU(), nn.Linear(512, output_dim)
        )
