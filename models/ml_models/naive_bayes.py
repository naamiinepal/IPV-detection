...
def main(args, logger):
    ...
    for ii in range(k_fold):
        ...
        assert (args.nb.vectorizer == "count") or (args.nb.vectorizer == "tfidf"), error_msg
        
        if args.nb.vectorizer == "count":
            # For text.
            vectorizer = CountVectorizer(lowercase = False,
                                        ngram_range = (1,2),
                                        max_features= args.nb.max_features,
                                        preprocessor = lambda x: x,
                                        tokenizer = lambda sentence : tokenize(sentence),
                                        encoding = "utf-8")
        
        elif args.nb.vectorizer == "tfidf":
            # For text.
        ...
        nb = MultinomialNB(alpha = args.nb.alpha)
    ...
    # To csv.
    cache_path = join(args.cache_dir, 'nb')
    if not exists(cache_path):
        os.mkdir(cache_path)
        
    record_df.to_csv(join(cache_path, f'nb_{str(args.nb.alpha)}_{str(args.nb.vectorizer)}_{str(args.nb.max_features)}_results.csv'))
